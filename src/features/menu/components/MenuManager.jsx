...
import { Plus, Pencil, Trash2, Coffee, FolderPlus, Gift, Upload, X, SlidersHorizontal, Clock3 } from 'lucide-react';
...
import PreArrivalSettingsDialog from '@/components/prearrival/PreArrivalSettingsDialog';
...
export default function MenuManager() {
  ...
  const [salonSetting, setSalonSetting] = useState(null);
  ...
  const handleSaveCategory = async (e) => {
    ...
    try {
      if (catForm.id) {
        ...
        await base44.entities.MenuCategory.create({ name: catForm.name, complimentary: false, display_order: categories.length, salon_id: salonId });
  ...
  const savePreArrivalSettings = async (nextSettings) => {
    const payload = { ...normalizePreArrivalSettings(nextSettings), salon_id: salonId };
    if (salonSetting?.id) {
      const updated = await base44.entities.SalonSetting.update(salonSetting.id, payload);
      setSalonSetting(updated);
    } else {
      const created = await base44.entities.SalonSetting.create(payload);
      setSalonSetting(created);
    }
    toast({ title: 'Pre-arrival settings saved' });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border/30 bg-muted/20 p-4">
        <div>
          <p className="font-medium">Guest pre-arrival ordering</p>
          <p className="text-sm text-muted-foreground">Same-day arrival rules, preparation time and salon timezone.</p>
        </div>
        <Button variant="outline" className="rounded-full" onClick={() => setPreArrivalOpen(true)}><Clock3 className="mr-2 h-4 w-4" />Configure</Button>
      </div>
      ...
      <PreArrivalSettingsDialog
        open={preArrivalOpen}
        onOpenChange={setPreArrivalOpen}
        settings={salonSetting}
        onSave={savePreArrivalSettings}
      />
