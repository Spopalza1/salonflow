...
export default function GuestMessagesPanel() {
  ...
  const requestedGuestMessageId = searchParams.get('guest_message') || searchParams.get('source');
  const { markNotificationsForGuestMessageRead } = useNotifications();
  ...
  useEffect(() => {
    if (!requestedGuestMessageId || !messages.length) return;
    const found = messages.find(m => m.id === requestedGuestMessageId);
    if (found) {
      setSelectedMsg(found);
      markNotificationsForGuestMessageRead(found.id).catch(() => {});
    }
  }, [requestedGuestMessageId, messages, markNotificationsForGuestMessageRead]);

  const openMessage = (msg) => {
    setSelectedMsg(msg);
    markNotificationsForGuestMessageRead(msg.id).catch(() => {});
  };
