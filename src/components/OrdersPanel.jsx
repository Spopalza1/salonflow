...
export default function OrdersPanel() {
  ...
  const { markNotificationsForSourceRead } = useNotifications();
  ...
  useEffect(() => {
    ...
    markNotificationsForSourceRead('order', requestedOrderId).catch(() => {});
    setTimeout(() => document.querySelector(`[data-order-id="${requestedOrderId}"]`)?.scrollIntoView({ behavior: 'smooth', block: 'center' }), 200);
  }, [requestedOrderId, markNotificationsForSourceRead]);

  const updateStatus = async (order, status) => {
    const prevStatus = order.status;
    markNotificationsForSourceRead('order', order.id).catch(() => {});
      ...
      {sorted.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          {sorted.map(order => (
            ...
            >
            <Card onClick={() => markNotificationsForSourceRead('order', order.id).catch(() => {})} className={`${order.status === 'pending' ? 'border-destructive/50' : ''} ${isArrivingSoon(order) ? 'border-amber-400 ring-1 ring-amber-400' : ''}`}>
