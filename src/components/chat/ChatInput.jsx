...
export default function ChatInput({ onSend }) {
  ...
  const [liveLevels, setLiveLevels] = useState(Array(28).fill(0.18));
  ...
  const capturedLevelsRef = useRef([]);
  ...
  const startAnalyser = (stream) => {
    ...
    const tick = () => {
      ...
      const level = Math.max(0.12, Math.min(1, data.reduce((a, b) => a + b, 0) / (data.length * 150)));
      setLiveLevels(prev => [...prev.slice(1), level]);
      capturedLevelsRef.current.push(level);
      if (capturedLevelsRef.current.length > 1000) capturedLevelsRef.current.shift();
  ...
  const startRecording = async (clientY) => {
    try {
      ...
      setLiveLevels(Array(28).fill(0.18));
  ...
  const handleSendVoice = async () => {
    ...
    try {
      ...
      await onSend({
        ...
        media_url: file_url,
        media_type: 'audio',
        audio_duration: recordTime,
        audio_mime_type: 'audio/wav',
        audio_size: recordedBlobRef.current.size,
        waveform_data: Array.from({ length: 42 }, (_, i) => {
          const samples = capturedLevelsRef.current;
          if (!samples.length) return 0.2;
          const index = Math.min(samples.length - 1, Math.floor((i / 41) * samples.length));
          return Number(Math.max(0.12, samples[index]).toFixed(2));
        }),
      });
      if (previewUrl) URL.revokeObjectURL(previewUrl);
      setPreviewUrl(null);
      ...
      {voiceState === 'preview' ? (
        ...
        <div className="glass-card rounded-2xl flex items-center gap-2 p-2">
          <div className="flex-1 flex items-center gap-2 px-3 py-2 rounded-xl bg-destructive/10 text-destructive">
            ...
            <div className="sf-live-waveform" aria-hidden="true">{liveLevels.map((level, i) => <span key={i} style={{ height: `${Math.max(18, level * 100)}%` }} />)}</div>
